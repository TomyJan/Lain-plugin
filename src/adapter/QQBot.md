...
请勿向购买任何人官方Bot资格，人财两空分分钟。 

目前QQBot仅对企业用户开放，个人开发者暂不开放。
...
title: QQ开放平台
desc: 有光的地方就有希望
logo: https://qqminiapp.cdn-go.cn/open-platform/f10fc34a/favicon.ico
link: https://q.qq.com
background: #fff8dc
```
...
```component VPCard
title: QQ机器人文档
desc: 正如影子随光而生，希望伴随着光明永存
logo: https://bot.q.qq.com/wiki/favicon.ico
link: https://bot.q.qq.com/wiki
background: #fff8dc
```

前往 `QQ开放平台`  -> `登录(企业)` -> `应用管理` -> `创建机器人`
...
| --------------------- | ------ | ---- | ---- |
| 机器人QQ号            | string | 否   |  |
| AppID (机器人ID)      | string | 是   |  |
| Token(机器人令牌)     | string | 是   | 机器人身份调用 openapi，后续废除 |
| AppSecret(机器人密钥) | string | 是   | 用于在 oauth 场景进行请求签名的密钥 |

![](https://cdn.jsdelivr.net/gh/Zyy955/imgs/img/202401201232847.png)
...
如果你没有登录QQ，可以在`CMD`等命令控制台直接执行指令，像QQ一样！
